...
For IP addressing, bLEO uses a private class A scheme (`10.X.Y.Z`). The rule for determining the address revolves around the `north` and `east` interfaces. The IP of the interface connecting the `IDsrc` (`north`/`east`) to `IDdst` (`south`/`west`) is determined as follow: byte `X` is set with the first 8 bits of the `IDsrc`. Byte `Y` is set with the first 8 bits of the `IDdst`. Finally, the last byte is set using the last three bits of the `IDsrc`, together with the last three bits of `IDdst`, together with `01` for the `north`/`east` interface or `02` for `south`/`west` interface. For example, IP of the interface `p02s15north` (`ID=64`, assuming `maxp`=12 and `maxs`=24) is `10.64.65.1` and the IP of the interface `p03s21west` (`ID=94`) is set following the rules for the interface `p02s21east` (`ID=70`) as `10.70.94.2`.

The `tracer` application is intended to show you how many packets have been sent for each link interface. This utility can be used to infer the path followed by the IP datagrams in a LEO network
...
Finally, the script `showifid` is a utility tool that shows the kernel identifier of every network link used by bLEO. This script is needed by `tracer` utility and can be helpful if you want to set up the link delay by hand using directly the utility `updatemap` (unfortunately, eBPF library does not understand link names but only link identifiers).
